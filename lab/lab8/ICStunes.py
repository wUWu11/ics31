##Deyu Wu 0952612. finish ICStunes by myself
##
...
def Song_str(S:Song)->str:
    '''takes a Song object and returns string in a readable fomart
    '''
    return str(S.track) + "---" +S.title +"---" +str(S.length) +'---play count:'+str(S.play_count) + '\n'
print(Song_str(Song(1, "Gimme Shelter", 272, 3)))
def Album_str(A:Album)->str:
    result = str(A.id) + ':  '+A.artist +'--' +A.title +'--'+ str(A.year) +'\n'
    for S in A.songs:
        result += Song_str(S)
    return result
...
def Songdisplay_str(Sd:Songdisplay)->str:
    '''takes a Songdisplay and returns a string containing that information in an easily readable
       form suitable for printing
    '''
    return str(Sd.track)+'  ' +Sd.a_title +'   ' + Sd.artist +'  '+str(Sd.year) +'  '+ str(Sd.length)+' ' +str(Sd.play_count)
for Sd in test_list:
    print(Songdisplay_str(Sd))

print('\n sorting \n\n\n\n')

def num_tracks(a:Album):
    return len(a.songs)
...
collection_sort(MUSIC,num_tracks)
print(MUSIC)#sorted collection

#e3
def unplayed_songs(MC:'list of Albums')->'list of Songplays':
    '''akes a music collection (a list of albums) and returns a list of
    Songdisplays, one for each song that has never been played. '''
    result = []
    for a in MC:
        for s in a.songs:
            if s.play_count == 0:
...
#e5
def favorite_album(MC:'list of Albums')->Album:
    '''t takes a list of albums and returns the album that is the "favorite."
    We'll define the favorite album as the one that the user has spent the
...
def top_n(MC: 'list of Album', n: int,func:'function',state:bool) -> 'list of Songdisplay':
    list_of_Songdisplays = all_Songdisplays(MC)
    list_of_Songdisplays.sort(key=func, reverse=state)
    return list_of_Songdisplays[:n]
...
def Album_listening_time(a:Album):
    return a.length*a.play_count
def Album_play_count(a:Album):
    return a.play_count
def favorite_album2(MC:'list of Albums',func:'function')->Album:
...
def collection_search(MC:'list of Albums',key:str)->list:
    '''taking a collection and a string as parameters and returning a list of Songdisplays
    of songs whose title, artist, or album title include that string. 
    '''
    result = []
    for a in MC:
        if key in a.title or key in a.artist:
            result.extend(Album_to_Songdisplays(a))
        for s in a.songs:
            if key in s.title:
                result.append(Songdisplay(a.artist, a.title, a.year,
...
print(collection_search(MUSIC,'Postal')) #search album title
print(collection_search(MUSIC,'Love')) #search song title
print(collection_search(MUSIC,'Midnight Oil')) #search author
